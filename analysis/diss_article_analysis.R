...
seedlings$Height.leaf.ratio <-
  (seedlings$Height.cm / seedlings$No.leaves)

# Designate as an upland or lowland species
seedlings <-  seedlings %>%
  mutate(
    hi_lo_species = case_when(
      Species == "AV" | Species == "CR" |
...
seedlings_melt_traits <-
  melt(
    seedlings,
    id.vars = c("Individual.code",
...
## Make the plot
trait_sp_ggplot <-
  ggplot(seedlings_melt_traits,
    aes(x = Species, y = value, colour = Species)) +
  geom_point() +
  facet_wrap( ~ variable, scales = "free")

# Plotting all seedlings by elevation ----
seedlings_comp$Elevation_rank <-
  rank(seedlings_comp$Elevation,
    na.last = TRUE,
    ties.method = "random")
seedlings_elev_ggplot <-
  ggplot(seedlings_comp,
    aes(x = Elevation_rank, y = Elevation, colour = Species)) + geom_point()
...
unique_elevation.code_species_Site <-
  unique(seedlings[c("Elevation.code", "Species", "Site")])
Top_range_site <-
  c("TU2",
    "TU7",
    "TU6",
    ...
    "VC1")
Middle_range_site <-
  c("TU4", "TU8", "TU7", "TU7", "TU6", "SP1", NA, NA, NA)
Bottom_range_site <-
  c(
    "TU8",
    "SP2",
...
Species_sample_loc <-
  data.frame(species, Top_range_site, Middle_range_site, Bottom_range_site)
...
Species_all_loc <-
  data.frame("species" = Species_all_3,
    "position" = Position_all_3,
    "range" = Range_all_3)

## How many species are at each elevation
species_elevcode_tally <-
  data.frame(table(seedlings_comp$Species, seedlings_comp$Elevation.code) [, ])
stargazer(species_elevcode_tally, type = "latex", summary = F)

# genus level migration rates plot
genus_mig_rates_ggplot <-
  ggplot(genus_mig_rates,
    aes(x = genus, y = migration.rate.m.yr.abund, fill = genus)) +
...
Annual_Precip <-
  c(NA, NA, 3087, 2631, 2631, 2472, 1827, NA, 2318, NA)
Annual_Air_Temp <-
  c(NA, NA, 20.7, 17.4, 15.8, 16, NA, 14.9, 11.1, 8.9)
...
ranges_ggplot <- ggplot() +
  geom_abline(
    aes(intercept = elev_mean, slope = 0),
    linetype = 2,
    size = 0.5,
    data = camp_loc
  ) +
  geom_point(aes(x = species, y = range, colour = position),
    size = 5,
    data = Species_all_loc) +
  geom_point(
    aes(x = species, y = range),
    shape = 15,
    size = 4,
    data = ranges
  ) +
...
seedlings_melt_env <-
  melt(
    seedlings_comp,
    id.vars = c("Individual.code", "Elevation.code", "Species", "Elevation"),
    measure.vars = c(
      "Soil.temp.mean",
      "Soil.mois.mean",
      "LAI.4.ring",
      "Comp.adult.total",
      "Comp.adult.metric",
      "Comp.adult.log.metric"
    )
  )

env_elev_box_all_ggplot <-
...
env_box_sp_ggplot <-
  ggplot(seedlings_melt_env,
    aes(
      x = Species,
      y = value,
      fill = interaction(Elevation.code, Species)
    )) +
...
(
  env_elev_scatter_ggplot <-
    ggplot(seedlings_melt_env, aes(
      x = Elevation, y = value, colour = Species
    )) +
    geom_point() +
    facet_wrap( ~ variable, scales = "free") +
    theme_classic() +
...
env_elev_lm_scatter_ggplot <-
  ggplot(seedlings_melt_env, aes(x = Elevation, y = value, colour = Species)) +
  geom_point() +
  geom_smooth(aes(fill = Species, colour = Species),
    method = lm,
    se = T) +
  facet_wrap( ~ variable, scales = "free")

phys_trait_elev_lm_scatter_ggplot <-
  ggplot(filter(seedlings_melt_traits, variable %in% c("D.FvFm", "SPAD.mean")),
...
Competition_Radius <-
  data.frame("site" = camp_loc$Site,
    "elev_mean" = camp_loc$elev_mean,
    k,
    trees_ha)
Competition_Radius$C_R <-
  Competition_Radius$k * (sqrt(10000 / trees_ha))

## Linear regression of trees / ha vs elevation to get VC
Competition_Radius_VC <-
  filter(
    Competition_Radius,
    site %in% c(
      "PA1",
...
Competition_Radius_K <-
  dplyr::select(Competition_Radius,-k,-elev_mean)
...
aberg_census_summ <- aberg_census %>%
  ...
  mutate(
    ...
    sampled = if_else(
      genus_species == "Alzatea verticillata" |
...
rank_abund_ggplot <- ggplot() +
  geom_point(
    data = filter(aberg_census_summ, sampled == T),
    aes(x = id, y = n),
    colour = "#E03A50FE",
    alpha = 1,
    size = 5
  ...
  geom_point(
    data = filter(aberg_census_summ, myrcia == T),
    aes(x = id, y = n),
    colour = "#53C90E",
    alpha = 1,
    size = 5
  ...
  geom_point(
    data = filter(aberg_census_summ, sampled == F),
    aes(x = id, y = n),
    colour = "black",
    alpha = 0.5,
    size = 1
...
soil_nitrogen_ggplot <-
  ggplot(soil, aes(x = site, y = c_per)) + geom_boxplot(aes(colour = site))

# Linear mixed models, competition versus elevation ----

## LAI.4.ring
LAI4ring_vs_elev_lmer <-
  lmer(LAI.4.ring ~ Elevation + (1 |
      Site),
    data = seedlings_rem_na,
    REML = F)
LAI4ring_vs_elev_lmer_slope <-
  lmer(LAI.4.ring ~ Elevation + (Elevation |
      Site),
    data = seedlings_rem_na,
    REML = F)
LAI4ring_vs_elev_lmer_rand <-
  lmer(LAI.4.ring ~ (1 | Site), data = seedlings_rem_na, REML = F)
LAI4ring_vs_elev_lmer_null <-
  lm(LAI.4.ring ~ 1, data = seedlings_rem_na)

## Adult competition metric
comp_adult_metric_vs_elev_lmer <-
  lmer(Comp.adult.metric ~ Elevation + (1 |
      Site),
    data = seedlings_rem_na,
    REML = F)
comp_adult_log_metric_vs_elev_lmer <-
  lmer(Comp.adult.log.metric ~ Elevation + (1 |
      Site),
    data = seedlings_rem_na,
    REML = F)
comp_adult_log_metric_vs_elev_lmer_slope <-
  lmer(
    Comp.adult.log.metric ~ Elevation + (Elevation |
        Site),
    data = seedlings_rem_na,
    REML = F
  )
comp_adult_log_metric_vs_elev_lmer_null <-
  lm(Comp.adult.log.metric ~ 1, data = seedlings_rem_na)
comp_adult_log_metric_vs_elev_lmer_rand <-
  lmer(Comp.adult.log.metric ~ (1 | Site), data = seedlings_rem_na)

## Soil temperature
soil_temp_vs_elev_lmer <-
  lmer(Soil.temp.mean ~ Elevation + (1 |
      Site),
    data = seedlings_rem_na,
    REML = F)
soil_temp_vs_elev_lmer_day <-
  lmer(
    Soil.temp.mean ~ Elevation + (1 |
        Site) + (1 | Collection.day),
    data = seedlings_rem_na,
    REML = F
  )

## Soil moisture
soil_mois_vs_elev_lmer <-
  lmer(Soil.mois.mean ~ Elevation + (1 |
      Site),
    data = seedlings_rem_na,
    REML = F)
soil_mois_vs_elev_lmer_null <-
  lmer(Soil.temp.mean ~ (1 | Site), data = seedlings_rem_na, REML = F)
soil_mois_vs_elev_date_lmer <-
  lmer(
    Soil.mois.mean ~ Elevation + (1 |
        Site) + (1 | Collection.date),
    data = seedlings_rem_na,
    REML = F
  )

### Best fitting models
stargazer(LAI4ring_vs_elev_lmer,
  comp_adult_log_metric_vs_elev_lmer,
  summary = F)
...
LAI4ring_var <-
  diag(matrix_LAI4ring %*% tcrossprod(vcov(LAI4ring_vs_elev_lmer), matrix_LAI4ring))
LAI4ring_pred <-
  data.frame(
    LAI4ring_pred,
    plo = LAI4ring_pred$LAI.4.ring - 2 * sqrt(LAI4ring_var),
    phi = LAI4ring_pred$LAI.4.ring + 2 * sqrt(LAI4ring_var)
  )

LAI4ring_plot <-
  ggplot(seedlings_rem_na, aes(x = Elevation, y = LAI.4.ring)) +
  geom_point(aes(colour = Site), alpha = 0.8) +
  geom_line(
    data = LAI4ring_pred,
    aes(x = Elevation, y = LAI.4.ring),
    stat = "identity",
    position = "identity"
  ) +
  geom_ribbon(
    data = LAI4ring_pred,
    aes(x = Elevation, ymin = plo, ymax = phi),
    stat = "identity",
    position = "identity",
    alpha = 0.6
  )
...
comp_adult_log_var <-
  diag(matrix_comp_adult_log_metric %*% tcrossprod(
    vcov(comp_adult_log_metric_vs_elev_lmer),
    matrix_comp_adult_log_metric
  ))
comp_adult_log_metric_pred <-
  data.frame(
    comp_adult_log_metric_pred,
    plo = comp_adult_log_metric_pred$comp_adult_log_metric - 2 * sqrt(comp_adult_log_var),
    phi = comp_adult_log_metric_pred$comp_adult_log_metric + 2 * sqrt(comp_adult_log_var)
  )

isi_plot <-
  ggplot(seedlings_rem_na, aes(x = Elevation, y = Comp.adult.log.metric)) +
  geom_point(aes(colour = Site), alpha = 0.8) +
  geom_line(
    data = comp_adult_log_metric_pred,
    aes(x = Elevation, y = (plo + phi) / 2),
    stat = "identity",
    position = "identity"
  ) +
  geom_ribbon(
...
soil_temp_var <-
  diag(matrix_soil_temp %*% tcrossprod(vcov(soil_temp_vs_elev_lmer), matrix_soil_temp))
soil_temp_pred <-
  data.frame(
    soil_temp_pred,
    plo = soil_temp_pred$soil_temp - 2 * sqrt(soil_temp_var),
    phi = soil_temp_pred$soil_temp + 2 * sqrt(soil_temp_var)
  )

temp_plot <-
  ggplot(seedlings_rem_na, aes(x = Elevation, y = Soil.temp.mean)) +
  geom_point(aes(colour = Site), alpha = 0.8) +
  geom_line(
    data = soil_temp_pred,
    aes(x = Elevation, y = (plo + phi) / 2),
...
matrix_soil_mois <-
  model.matrix(terms(soil_mois_vs_elev_lmer), data = soil_mois_pred)
soil_mois_pred$soil_mois = matrix_soil_mois %*% fixef(soil_mois_vs_elev_lmer)
soil_mois_var <-
  diag(matrix_soil_mois %*% tcrossprod(vcov(soil_mois_vs_elev_lmer), matrix_soil_mois))
soil_mois_pred <-
  data.frame(
    soil_mois_pred,
    plo = soil_mois_pred$soil_mois - 2 * sqrt(soil_mois_var),
    phi = soil_mois_pred$soil_mois + 2 * sqrt(soil_mois_var)
  )

mois_plot <-
  ggplot(seedlings_rem_na, aes(x = Elevation, y = Soil.mois.mean)) +
  geom_point(aes(colour = Site), alpha = 0.8) +
  geom_line(
    data = soil_mois_pred,
    aes(x = Elevation, y = (plo + phi) / 2),
...
for (i in unique(lm_sp_slope$trait)) {
  interv_hi_lo_listggplot[[i]] <-
    ggplot(filter(lm_sp_slope, trait == i), aes(x = Species)) +
...
stdz.mod_fvfm_elev_ri <-
  standardize(lmer(
    ...
    standardize.y = T)
stdz.mod_fvfm_elev_rs <-
  standardize(lmer(
    D.FvFm ~ Elevation.code + (Elevation.code |
        Species) + (1 |
            Site),
    data = seedlings_rem_na,
    REML = F
...
stdz.mod_fvfm_lai_ri <-
  standardize(lmer(
    ...
    standardize.y = T)
stdz.mod_fvfm_lai_rs <-
  standardize(lmer(
    D.FvFm ~ LAI.4.ring + (LAI.4.ring |
        Species) + (1 |
            Site),
    data = seedlings_rem_na,
    REML = F
...
stdz.mod_fvfm_isi_ri <-
  standardize(
    lmer(
      D.FvFm ~ Comp.adult.log.metric + (1 |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_fvfm_isi_rs <-
  standardize(
    lmer(
      D.FvFm ~ Comp.adult.log.metric + (Comp.adult.log.metric |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_chl_elev_ri <-
  standardize(lmer(
    chl ~ Elevation.code + (1 |
        Species) + (1 |
            Site),
    data = seedlings_rem_na,
    REML = F
    ...
    standardize.y = T)
stdz.mod_chl_elev_rs <-
  standardize(lmer(
    chl ~ Elevation.code + (Elevation.code |
        Species) + (1 |
            Site),
    data = seedlings_rem_na,
    REML = F
...
stdz.mod_chl_isi_rs <-
  standardize(
    lmer(
      chl ~ Comp.adult.log.metric + (Comp.adult.log.metric |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_thick_elev_ri <-
  standardize(
    lmer(
      Leaf.thick.mean.mm ~ Elevation.code + (1 |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_thick_elev_rs <-
  standardize(
    lmer(
      Leaf.thick.mean.mm ~ Elevation.code + (Elevation.code |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_thick_lai_ri <-
  standardize(
    lmer(
      Leaf.thick.mean.mm ~ LAI.4.ring + (1 |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_thick_lai_rs <-
  standardize(
    lmer(
      Leaf.thick.mean.mm ~ LAI.4.ring + (LAI.4.ring |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_thick_isi_ri <-
  standardize(
    lmer(
      Leaf.thick.mean.mm ~ Comp.adult.log.metric + (1 |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_thick_isi_rs <-
  standardize(
    lmer(
      Leaf.thick.mean.mm ~ Comp.adult.log.metric + (Comp.adult.log.metric |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_hlratio_elev_ri <-
  standardize(
    lmer(
      Height.leaf.ratio ~ Elevation.code + (1 |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_hlratio_elev_rs <-
  standardize(
    lmer(
      Height.leaf.ratio ~ Elevation.code + (Elevation.code |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_hlratio_lai_ri <-
  standardize(
    lmer(
      Height.leaf.ratio ~ LAI.4.ring + (1 |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_hlratio_lai_rs <-
  standardize(
    lmer(
      Height.leaf.ratio ~ LAI.4.ring + (LAI.4.ring |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_hlratio_isi_ri <-
  standardize(
    lmer(
      Height.leaf.ratio ~ Comp.adult.log.metric + (1 |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_hlratio_isi_rs <-
  standardize(
    lmer(
      Height.leaf.ratio ~ Comp.adult.log.metric + (Comp.adult.log.metric |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_area_elev_ri <-
  standardize(lmer(
    Leaf.area ~ Elevation.code + (1 |
        Species) + (1 |
            Site),
    data = seedlings_rem_na,
    REML = F
...
stdz.mod_area_elev_rs <-
  standardize(lmer(
    Leaf.area ~ Elevation.code + (Elevation.code |
        Species) + (1 |
            Site),
    data = seedlings_rem_na,
    REML = F
...
stdz.mod_area_lai_ri <-
  standardize(lmer(
    Leaf.area ~ LAI.4.ring + (1 |
        Species) + (1 |
            Site),
    data = seedlings_rem_na,
    REML = F
...
stdz.mod_area_lai_rs <-
  standardize(lmer(
    Leaf.area ~ LAI.4.ring + (LAI.4.ring |
        Species) + (1 |
            Site),
    data = seedlings_rem_na,
    REML = F
...
stdz.mod_area_isi_ri <-
  standardize(
    lmer(
      Leaf.area ~ Comp.adult.log.metric + (1 |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_area_isi_rs <-
  standardize(
    lmer(
      Leaf.area ~ Comp.adult.log.metric + (Comp.adult.log.metric |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_stemvol_elev_ri <-
  standardize(
    lmer(
      Stem.volume.cm3 ~ Elevation.code + (1 |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_stemvol_elev_rs <-
  standardize(
    lmer(
      Stem.volume.cm3 ~ Elevation.code + (Elevation.code |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_stemvol_lai_rs <-
  standardize(
    lmer(
      Stem.volume.cm3 ~ LAI.4.ring + (LAI.4.ring |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_stemvol_isi_ri <-
  standardize(
    lmer(
      Stem.volume.cm3 ~ Comp.adult.log.metric + (1 |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_stemvol_isi_rs <-
  standardize(
    lmer(
      Stem.volume.cm3 ~ Comp.adult.log.metric + (Comp.adult.log.metric |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_fvfm_null <-
  standardize(lmer(D.FvFm ~ (1 |
      Species) + (1 | Site), data = seedlings_rem_na), standardize.y = T)
stdz.mod_chl_null <-
  standardize(lmer(chl ~ (1 |
      Species) + (1 | Site), data = seedlings_rem_na), standardize.y = T)
stdz.mod_thick_null <-
  standardize(lmer(Leaf.thick.mean.mm ~ (1 |
      Species) + (1 | Site), data = seedlings_rem_na),
    standardize.y = T)
stdz.mod_hlratio_null <-
  standardize(lmer(Height.leaf.ratio ~ (1 |
      Species) + (1 | Site), data = seedlings_rem_na),
    standardize.y = T)
stdz.mod_area_null <-
  standardize(lmer(Leaf.area ~ (1 |
      Species) + (1 | Site), data = seedlings_rem_na), standardize.y = T)
stdz.mod_stemvol_null <-
  standardize(lmer(Stem.volume.cm3 ~ (1 |
      Species) + (1 | Site), data = seedlings_rem_na),
    standardize.y = T)
AIC_ri <- c(
...
daic13 <-
  AIC(stdz.mod_hlratio_null) - AIC(stdz.mod_hlratio_elev_rs)
...
daic21 <-
  AIC(stdz.mod_stemvol_null) - AIC(stdz.mod_stemvol_elev_ri)
...
slope1 <-
  unname(coef(summary(stdz.mod_fvfm_elev_ri)) [, "Estimate"])
slope2 <-
  unname(coef(summary(stdz.mod_fvfm_lai_ri)) [, "Estimate"])
slope4 <-
  unname(coef(summary(stdz.mod_fvfm_isi_ri)) [, "Estimate"])
slope5 <-
  unname(coef(summary(stdz.mod_chl_elev_ri)) [, "Estimate"])
slope6 <-
  unname(coef(summary(stdz.mod_chl_lai_ri)) [, "Estimate"])
slope8 <-
  unname(coef(summary(stdz.mod_chl_isi_ri)) [, "Estimate"])
slope9 <-
  unname(coef(summary(stdz.mod_thick_elev_ri)) [, "Estimate"])
slope10 <-
  unname(coef(summary(stdz.mod_thick_lai_rs)) [, "Estimate"])
slope12 <-
  unname(coef(summary(stdz.mod_thick_isi_ri)) [, "Estimate"])
slope13 <-
  unname(coef(summary(stdz.mod_hlratio_elev_rs)) [, "Estimate"])
slope14 <-
  unname(coef(summary(stdz.mod_hlratio_lai_ri)) [, "Estimate"])
slope16 <-
  unname(coef(summary(stdz.mod_hlratio_isi_ri)) [, "Estimate"])
slope17 <-
  unname(coef(summary(stdz.mod_area_elev_rs)) [, "Estimate"])
slope18 <-
  unname(coef(summary(stdz.mod_area_lai_rs)) [, "Estimate"])
slope20 <-
  unname(coef(summary(stdz.mod_area_isi_rs)) [, "Estimate"])
slope21 <-
  unname(coef(summary(stdz.mod_stemvol_elev_ri)) [, "Estimate"])
slope22 <-
  unname(coef(summary(stdz.mod_stemvol_lai_ri)) [, "Estimate"])
slope24 <-
  unname(coef(summary(stdz.mod_stemvol_isi_rs)) [, "Estimate"])
...
explan_best <-
  data.frame(trait_name_best,
    ...
    r2m_best)
explan_best
explan_best$trait_name_best <-
  factor(explan_best$trait_name_best, levels = explan_best$trait_name_best)
...
# dAIC
daicplot_ggplot <-
  ggplot(explan_best_phys,
    aes(
      x = trait_name_best,
      y = dAIC_best,
      group = factor(model_name_best)
    )) +
  geom_bar(
    stat = "identity",
    position = position_dodge(),
    aes(fill = model_name_best),
    colour = "black"
  ) +
  theme(legend.position = "right") +
  theme(legend.title = element_blank()) +
  scale_fill_discrete(
    breaks = c("elev", "isi", "lai"),
    labels = c("Elevation.code", "ISI", "LAI")
  ) +
  scale_x_discrete(
    breaks = c(
      "D.FvFm",
...
# R^2
r2plot_ggplot <-
  ggplot(explan_best_phys, aes(x = trait_name_best, group = factor(model_name_best)))  +
  geom_bar(
    stat = "identity",
    position = position_dodge(),
    aes(y = r2c_best, fill = model_name_best),
    colour = "black",
    alpha = 0.2
  ) +
  geom_bar(
    stat = "identity",
    position = position_dodge(),
    aes(y = r2m_best, fill = model_name_best),
    colour = "black"
  ) +
  theme(legend.position = "right") +
  theme(legend.title = element_blank()) +
  ...
  ylab(expression(paste(
    "Variance explained by fixed effect (", R[C] ^ 2, ")"
  ))) +
  scale_x_discrete(
    breaks = c(
      "D.FvFm",
...
single_trait_eff_size_lmer_ggplot <- ggplot(data = explan_best) +
  geom_errorbar(
    aes(
      x = model_name_best,
      ymin = slope_best - ss_best,
      ymax = slope_best + ss_best,
      colour = factor(model_name_best)
    )
  ) +
  geom_point(aes(
    x = model_name_best,
  ...
  geom_hline(yintercept = 0, linetype = 5) +
  scale_x_discrete("model_name_best",
    labels = c(
      "elev" = "Elevation.code",
      "isi" = "ISI",
      "lai" = "LAI"
...
stdz.mod_fvfm_full <-
  standardize(
    lmer(
      D.FvFm ~ LAI.4.ring + Comp.seed.total + Comp.adult.log.metric + Elevation.code + (1 |
          Species) + (1 |
              Site) ,
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_fvfm_lai_seed_isi <-
  standardize(
    lmer(
      D.FvFm ~ LAI.4.ring + Comp.seed.total + Comp.adult.log.metric + (1 |
          Species) + (1 |
              Site) ,
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_fvfm_lai_seed_elev <-
  standardize(
    lmer(
      D.FvFm ~ LAI.4.ring + Comp.seed.total + Elevation.code + (1 |
          Species) + (1 |
              Site) ,
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_fvfm_lai_isi_elev <-
  standardize(
    lmer(
      D.FvFm ~ LAI.4.ring + Comp.adult.log.metric + Elevation.code + (1 |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_fvfm_seed_isi_elev <-
  standardize(
    lmer(
      D.FvFm ~ Comp.seed.total + Comp.adult.log.metric + Elevation.code + (1 |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_fvfm_aic_val <- c(
  AIC(stdz.mod_fvfm_full),
...
model_name_stdz.mod_fvfm_aic <- c(
  "stdz.mod_fvfm_full",
  "stdz.mod_fvfm_lai_seed_isi",
  "stdz.mod_fvfm_lai_seed_elev",
  "stdz.mod_fvfm_lai_isi_elev",
...
stdz.mod_fvfm_aic <-
  data.frame(model_name_stdz.mod_fvfm_aic, stdz.mod_fvfm_aic_val)
stdz.mod_fvfm_aic_arr <-
  arrange(stdz.mod_fvfm_aic, stdz.mod_fvfm_aic_val)
stdz.mod_fvfm_aic_arr
stdz.mod_fvfm_aic_arr$dAIC <-
  c(
    AIC(stdz.mod_fvfm_lai_isi_elev) - AIC(stdz.mod_fvfm_rand),
...
stdz.mod_fvfm_wi <-
  Weights(stdz.mod_fvfm_aic_arr$stdz.mod_fvfm_aic_val)
stdz.mod_fvfm_wi
stdz.mod_fvfm_aic_arr$wi <- stdz.mod_fvfm_wi
stdz.mod_fvfm_aic_arr <-
  stdz.mod_fvfm_aic_arr %>% dplyr::select(model_name_stdz.mod_fvfm_aic, stdz.mod_fvfm_aic_val, dAIC, wi)
...
stdz.mod_fvfm_aic_arr$R2c <-
  c(r1[2], r2[2], r3[2], r4[2], r5[2], r6[2], r7[2], r8[2], r9[2], r10[2])
stdz.mod_fvfm_aic_arr$r2m <-
  c(r1[1], r2[1], r3[1], r4[1], r5[1], r6[1], r7[1], r8[1], r9[1], r10[1])
...
stdz.mod_fvfm_fix_name <-
  c("LAI", "ISI", "Elevation.code", "Comp.seed")
...
stdz.mod_fvfm_fix <-
  data.frame(
    "factor" = stdz.mod_fvfm_fix_name,
    "slope" = stdz.mod_fvfm_fix_slope,
    "se" = stdz.mod_fvfm_fix_se,
    "trait" = stdz.mod_fvfm_fix_trait
  )
stdz.mod_fvfm_fix
stdz.mod_fvfm_sort <- c("Elevation.code", "ISI", "LAI", "Comp.seed")
stdz.mod_fvfm_fix <-
  stdz.mod_fvfm_fix %>% slice(match(stdz.mod_fvfm_sort, stdz.mod_fvfm_fix_name))
stdz.mod_fvfm_fix$stdz.mod_fvfm_fix_name <-
  factor(stdz.mod_fvfm_fix$stdz.mod_fvfm_fix_name,
    levels = stdz.mod_fvfm_fix$stdz.mod_fvfm_fix_name)
stdz.mod_fvfm_fix
...
stdz.mod_spad_full <-
  standardize(
    lmer(
      SPAD.mean ~ LAI.4.ring + Comp.seed.total + Comp.adult.log.metric + Elevation.code + (1 |
          Species) + (1 |
              Site) ,
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_spad_lai_seed_isi <-
  standardize(
    lmer(
      SPAD.mean ~ LAI.4.ring + Comp.seed.total + Comp.adult.log.metric + (1 |
          Species) + (1 |
              Site) ,
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_spad_lai_seed_elev <-
  standardize(
    lmer(
      SPAD.mean ~ LAI.4.ring + Comp.seed.total + Elevation.code + (1 |
          Species) + (1 |
              Site) ,
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_spad_lai_isi_elev <-
  standardize(
    lmer(
      SPAD.mean ~ LAI.4.ring + Comp.adult.log.metric + Elevation.code + (1 |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_spad_seed_isi_elev <-
  standardize(
    lmer(
      SPAD.mean ~ Comp.seed.total + Comp.adult.log.metric + Elevation.code + (1 |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_spad_aic_val <- c(
  AIC(stdz.mod_spad_full),
...
model_name_stdz.mod_spad_aic <- c(
  "stdz.mod_spad_full",
  "stdz.mod_spad_lai_seed_isi",
  "stdz.mod_spad_lai_seed_elev",
  "stdz.mod_spad_lai_isi_elev",
...
stdz.mod_spad_aic <-
  data.frame(model_name_stdz.mod_spad_aic, stdz.mod_spad_aic_val)
stdz.mod_spad_aic_arr <-
  arrange(stdz.mod_spad_aic, stdz.mod_spad_aic_val)
stdz.mod_spad_aic_arr
stdz.mod_spad_aic_arr$dAIC <-
  c(
    AIC(stdz.mod_spad_rand) - AIC(stdz.mod_spad_rand),
...
stdz.mod_spad_aic_arr
stdz.mod_spad_wi <-
  Weights(stdz.mod_spad_aic_arr$stdz.mod_spad_aic_val)
stdz.mod_spad_wi
stdz.mod_spad_aic_arr$wi <- stdz.mod_spad_wi
stdz.mod_spad_aic_arr <-
  stdz.mod_spad_aic_arr %>% dplyr::select(model_name_stdz.mod_spad_aic, stdz.mod_spad_aic_val, dAIC, wi)
...
stdz.mod_spad_aic_arr$R2c <-
  c(r1[2], r2[2], r3[2], r4[2], r5[2], r6[2], r7[2], r8[2], r9[2], r10[2])
stdz.mod_spad_aic_arr$r2m <-
  c(r1[1], r2[1], r3[1], r4[1], r5[1], r6[1], r7[1], r8[1], r9[1], r10[1])
...
stdz.mod_spad_fix <-
  data.frame(
    "factor" = stdz.mod_spad_fix_name,
    "slope" = stdz.mod_spad_fix_slope,
    "se" = stdz.mod_spad_fix_se,
    "trait" = stdz.mod_spad_fix_trait
  )
stdz.mod_spad_fix
stdz.mod_spad_sort <- c("Elevation.code", "ISI", "LAI", "Comp.seed")
stdz.mod_spad_fix <-
  stdz.mod_spad_fix %>% slice(match(stdz.mod_spad_sort, stdz.mod_spad_fix_name))
stdz.mod_spad_fix$stdz.mod_spad_fix_name <-
  factor(stdz.mod_spad_fix$stdz.mod_spad_fix_name,
    levels = stdz.mod_spad_fix$stdz.mod_spad_fix_name)
stdz.mod_spad_fix

#Height.leaf.ratio
stdz.mod_hlratio_full <-
  standardize(
    lmer(
      Height.leaf.ratio ~ LAI.4.ring + Comp.seed.total + Comp.adult.log.metric + Elevation.code + (1 |
          Species) + (1 |
              Site) ,
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_hlratio_int <-
  standardize(
    lmer(
      Height.leaf.ratio ~ LAI.4.ring + Comp.seed.total + Comp.adult.log.metric + Elevation.code + (Elevation.code + Comp.seed.total |
          Species) + (1 |
              Site) ,
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_hlratio_lai_seed_isi <-
  standardize(
    lmer(
      Height.leaf.ratio ~ LAI.4.ring + Comp.seed.total + Comp.adult.log.metric + (1 |
          Species) + (1 |
              Site) ,
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_hlratio_lai_seed_elev <-
  standardize(
    lmer(
      Height.leaf.ratio ~ LAI.4.ring + Comp.seed.total + Elevation.code + (1 |
          Species) + (1 |
              Site) ,
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_hlratio_lai_isi_elev <-
  standardize(
    lmer(
      Height.leaf.ratio ~ LAI.4.ring + Comp.adult.log.metric + Elevation.code + (1 |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_hlratio_seed_isi_elev <-
  standardize(
    lmer(
      Height.leaf.ratio ~ Comp.seed.total + Comp.adult.log.metric + Elevation.code + (1 |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_hlratio_null <-
  lm(Height.leaf.ratio ~ 1, data = seedlings_rem_na)

stdz.mod_hlratio_aic_val <- c(
  AIC(stdz.mod_hlratio_full),
...
model_name_stdz.mod_hlratio_aic <- c(
  "stdz.mod_hlratio_full",
  "stdz.mod_hlratio_int",
  "stdz.mod_hlratio_lai_seed_isi",
  "stdz.mod_hlratio_lai_seed_elev",
...
stdz.mod_hlratio_aic <-
  data.frame(model_name_stdz.mod_hlratio_aic, stdz.mod_hlratio_aic_val)
stdz.mod_hlratio_aic_arr <-
  arrange(stdz.mod_hlratio_aic, stdz.mod_hlratio_aic_val)
stdz.mod_hlratio_aic_arr
stdz.mod_hlratio_aic_arr$dAIC <-
  c(
    AIC(stdz.mod_hlratio_int) - AIC(stdz.mod_hlratio_rand),
...
stdz.mod_hlratio_aic_arr
stdz.mod_hlratio_wi <-
  Weights(stdz.mod_hlratio_aic_arr$stdz.mod_hlratio_aic_val)
stdz.mod_hlratio_wi
stdz.mod_hlratio_aic_arr$wi <- stdz.mod_hlratio_wi
stdz.mod_hlratio_aic_arr <-
  stdz.mod_hlratio_aic_arr %>% dplyr::select(model_name_stdz.mod_hlratio_aic,
    stdz.mod_hlratio_aic_val,
    dAIC,
    wi)
...
stdz.mod_hlratio_aic_arr$R2c <-
  c(r1[2], r2[2], r3[2], r4[2], r5[2], r6[2], r7[2], r7[2], r9[2], r10[2], r11[2])
stdz.mod_hlratio_aic_arr$r2m <-
  c(r1[1], r2[1], r3[1], r4[1], r5[1], r6[1], r7[1], r8[1], r9[1], r10[1], r11[1])
...
#Creating fixed effect slope graph with standard error bars
stdz.mod_hlratio_fix_name <-
  c("LAI", "ISI", "Elevation.code", "Comp.seed")
stdz.mod_hlratio_fix_slope <-
  c(-0.10258, 0.01219,-0.14145 , 0.02634)
stdz.mod_hlratio_fix_se <- c(0.05014, 0.04830, 0.12770, 0.08709)
stdz.mod_hlratio_fix_trait <- rep("hlratio", 4)
stdz.mod_hlratio_fix <-
  data.frame(
    "factor" = stdz.mod_hlratio_fix_name,
    "slope" = stdz.mod_hlratio_fix_slope,
    "se" = stdz.mod_hlratio_fix_se,
    "trait" = stdz.mod_hlratio_fix_trait
  )
stdz.mod_hlratio_fix
stdz.mod_hlratio_sort <-
  c("Elevation.code", "ISI", "LAI", "Comp.seed")
stdz.mod_hlratio_fix <-
  stdz.mod_hlratio_fix %>% slice(match(stdz.mod_hlratio_sort, stdz.mod_hlratio_fix_name))
stdz.mod_hlratio_fix$stdz.mod_hlratio_fix_name <-
  factor(
    stdz.mod_hlratio_fix$stdz.mod_hlratio_fix_name,
    levels = stdz.mod_hlratio_fix$stdz.mod_hlratio_fix_name
  )
stdz.mod_hlratio_fix
...
stdz.mod_area_full <-
  standardize(
    lmer(
      Leaf.area ~ LAI.4.ring + Comp.seed.total + Comp.adult.log.metric + Elevation.code + (1 |
          Species) + (1 |
              Site) ,
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_area_int <-
  standardize(
    lmer(
      Leaf.area ~ LAI.4.ring + Comp.seed.total + Comp.adult.log.metric + Elevation.code + (1 + LAI.4.ring + Comp.seed.total + Elevation.code |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_area_lai_seed_isi <-
  standardize(
    lmer(
      Leaf.area ~ LAI.4.ring + Comp.seed.total + Comp.adult.log.metric + (1 |
          Species) + (1 |
              Site) ,
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_area_lai_seed_elev <-
  standardize(
    lmer(
      Leaf.area ~ LAI.4.ring + Comp.seed.total + Elevation.code + (1 |
          Species) + (1 |
              Site) ,
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_area_lai_isi_elev <-
  standardize(
    lmer(
      Leaf.area ~ LAI.4.ring + Comp.adult.log.metric + Elevation.code + (1 |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_area_seed_isi_elev <-
  standardize(
    lmer(
      Leaf.area ~ Comp.seed.total + Comp.adult.log.metric + Elevation.code + (1 |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_area_aic_val <- c(
  AIC(stdz.mod_area_full),
...
model_name_stdz.mod_area_aic <- c(
  "stdz.mod_area_full",
  "stdz.mod_area_int",
  "stdz.mod_area_lai_seed_isi",
  "stdz.mod_area_lai_seed_elev",
...
stdz.mod_area_aic <-
  data.frame(model_name_stdz.mod_area_aic, stdz.mod_area_aic_val)
stdz.mod_area_aic_arr <-
  arrange(stdz.mod_area_aic, stdz.mod_area_aic_val)
stdz.mod_area_aic_arr
stdz.mod_area_aic_arr$dAIC <-
  c(
    AIC(stdz.mod_area_lai_rs) - AIC(stdz.mod_area_rand),
...
stdz.mod_area_aic_arr
stdz.mod_area_wi <-
  Weights(stdz.mod_area_aic_arr$stdz.mod_area_aic_val)
stdz.mod_area_wi
stdz.mod_area_aic_arr$wi <- stdz.mod_area_wi
stdz.mod_area_aic_arr <-
  stdz.mod_area_aic_arr %>% dplyr::select(model_name_stdz.mod_area_aic, stdz.mod_area_aic_val, dAIC, wi)
...
stdz.mod_area_aic_arr$R2c <-
  c(r1[2], r2[2], r3[2], r4[2], r5[2], r6[2], r7[2], r8[2], r9[2], r10[2], r11[2])
stdz.mod_area_aic_arr$r2m <-
  c(r1[1], r2[1], r3[1], r4[1], r5[1], r6[1], r7[1], r8[1], r9[1], r10[1], r11[1])
...
stdz.mod_area_fix <-
  data.frame(
    "factor" = stdz.mod_area_fix_name,
    "slope" = stdz.mod_area_fix_slope,
    "se" = stdz.mod_area_fix_se,
    "trait" = stdz.mod_area_fix_trait
  )
stdz.mod_area_fix
stdz.mod_area_sort <- c("Elevation.code", "ISI", "LAI", "Comp.seed")
stdz.mod_area_fix <-
  stdz.mod_area_fix %>% slice(match(stdz.mod_area_sort, stdz.mod_area_fix_name))
stdz.mod_area_fix$stdz.mod_area_fix_name <-
  factor(stdz.mod_area_fix$stdz.mod_area_fix_name,
    levels = stdz.mod_area_fix$stdz.mod_area_fix_name)

#Stem Volume
stdz.mod_stemvol_full <-
  standardize(
    lmer(
      Stem.volume.cm3 ~ LAI.4.ring + Comp.seed.total + Comp.adult.log.metric + Elevation.code + (1 |
          Species) + (1 |
              Site) ,
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_stemvol_int <-
  standardize(
    lmer(
      Stem.volume.cm3 ~ LAI.4.ring + Comp.seed.total + Comp.adult.log.metric + Elevation.code + (1 + Comp.adult.log.metric |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_stemvol_lai_seed_isi <-
  standardize(
    lmer(
      Stem.volume.cm3 ~ LAI.4.ring + Comp.seed.total + Comp.adult.log.metric + (1 |
          Species) + (1 |
              Site) ,
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_stemvol_lai_seed_elev <-
  standardize(
    lmer(
      Stem.volume.cm3 ~ LAI.4.ring + Comp.seed.total + Elevation.code + (1 |
          Species) + (1 |
              Site) ,
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_stemvol_lai_isi_elev <-
  standardize(
    lmer(
      Stem.volume.cm3 ~ LAI.4.ring + Comp.adult.log.metric + Elevation.code + (1 |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_stemvol_seed_isi_elev <-
  standardize(
    lmer(
      Stem.volume.cm3 ~ Comp.seed.total + Comp.adult.log.metric + Elevation.code + (1 |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_stemvol_null <-
  lm(Stem.volume.cm3 ~ 1, data = seedlings_rem_na)

stdz.mod_stemvol_aic_val <- c(
  AIC(stdz.mod_stemvol_full),
...
model_name_stdz.mod_stemvol_aic <- c(
  "stdz.mod_stemvol_full",
  "stdz.mod_stemvol_int",
  "stdz.mod_stemvol_lai_seed_isi",
  "stdz.mod_stemvol_lai_seed_elev",
...
stdz.mod_stemvol_aic <-
  data.frame(model_name_stdz.mod_stemvol_aic, stdz.mod_stemvol_aic_val)
stdz.mod_stemvol_aic_arr <-
  arrange(stdz.mod_stemvol_aic, stdz.mod_stemvol_aic_val)
stdz.mod_stemvol_aic_arr
stdz.mod_stemvol_aic_arr$dAIC <-
  c(
    AIC(stdz.mod_stemvol_int) - AIC(stdz.mod_stemvol_rand),
...
stdz.mod_stemvol_aic_arr
stdz.mod_stemvol_wi <-
  Weights(stdz.mod_stemvol_aic_arr$stdz.mod_stemvol_aic_val)
stdz.mod_stemvol_wi
stdz.mod_stemvol_aic_arr$wi <- stdz.mod_stemvol_wi
stdz.mod_stemvol_aic_arr <-
  stdz.mod_stemvol_aic_arr %>% dplyr::select(model_name_stdz.mod_stemvol_aic,
    stdz.mod_stemvol_aic_val,
    dAIC,
    wi)
...
stdz.mod_stemvol_aic_arr$R2c <-
  c(r1[2], r2[2], r3[2], r4[2], r5[2], r6[2], r7[2], r8[2], r9[2], r10[2], r11[2])
stdz.mod_stemvol_aic_arr$r2m <-
  c(r1[1], r2[1], r3[1], r4[1], r5[1], r6[1], r7[1], r8[1], r9[1], r10[1], r11[1])
...
#Creating fixed effect slope graph with standard error bars
stdz.mod_stemvol_fix_name <-
  c("LAI", "ISI", "Elevation.code", "Comp.seed")
stdz.mod_stemvol_fix_slope <-
  c(-0.05366,-0.19821, 0.31046,-0.06353)
stdz.mod_stemvol_fix_se <- c(0.05709, 0.11863, 0.13184, 0.05322)
stdz.mod_stemvol_fix_trait <- rep("stemvol", 4)
stdz.mod_stemvol_fix <-
  data.frame(
    "factor" = stdz.mod_stemvol_fix_name,
    "slope" = stdz.mod_stemvol_fix_slope,
    "se" = stdz.mod_stemvol_fix_se,
    "trait" = stdz.mod_stemvol_fix_trait
  )
stdz.mod_stemvol_fix
stdz.mod_stemvol_sort <-
  c("Elevation.code", "ISI", "LAI", "Comp.seed")
stdz.mod_stemvol_fix <-
  stdz.mod_stemvol_fix %>% slice(match(stdz.mod_stemvol_sort, stdz.mod_stemvol_fix_name))
stdz.mod_stemvol_fix$stdz.mod_stemvol_fix_name <-
  factor(
    stdz.mod_stemvol_fix$stdz.mod_stemvol_fix_name,
    levels = stdz.mod_stemvol_fix$stdz.mod_stemvol_fix_name
  )
stdz.mod_stemvol_fix
...
stdz.mod_thick_full <-
  standardize(
    lmer(
      Leaf.thick.mean.mm ~ LAI.4.ring + Comp.seed.total + Comp.adult.log.metric + Elevation.code + (1 |
          Species) + (1 |
              Site) ,
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_thick_int <-
  standardize(
    lmer(
      Leaf.thick.mean.mm ~ LAI.4.ring + Comp.seed.total + Comp.adult.log.metric + Elevation.code + (1 + Comp.seed.total |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_thick_lai_seed_isi <-
  standardize(
    lmer(
      Leaf.thick.mean.mm ~ LAI.4.ring + Comp.seed.total + Comp.adult.log.metric + (1 |
          Species) + (1 |
              Site) ,
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_thick_lai_seed_elev <-
  standardize(
    lmer(
      Leaf.thick.mean.mm ~ LAI.4.ring + Comp.seed.total + Elevation.code + (1 |
          Species) + (1 |
              Site) ,
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_thick_lai_isi_elev <-
  standardize(
    lmer(
      Leaf.thick.mean.mm ~ LAI.4.ring + Comp.adult.log.metric + Elevation.code + (1 |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_thick_seed_isi_elev <-
  standardize(
    lmer(
      Leaf.thick.mean.mm ~ Comp.seed.total + Comp.adult.log.metric + Elevation.code + (1 |
          Species) + (1 |
              Site),
      data = seedlings_rem_na,
      REML = F
...
stdz.mod_thick_null <-
  lm(Leaf.thick.mean.mm ~ 1, data = seedlings_rem_na)

stdz.mod_thick_aic_val <- c(
  AIC(stdz.mod_thick_full),
...
model_name_stdz.mod_thick_aic <- c(
  "stdz.mod_thick_full",
  "stdz.mod_thick_int",
  "stdz.mod_thick_lai_seed_isi",
  "stdz.mod_thick_lai_seed_elev",
...
stdz.mod_thick_aic <-
  data.frame(model_name_stdz.mod_thick_aic, stdz.mod_thick_aic_val)
stdz.mod_thick_aic_arr <-
  arrange(stdz.mod_thick_aic, stdz.mod_thick_aic_val)
stdz.mod_thick_aic_arr
stdz.mod_thick_aic_arr$dAIC <-
  c(
    AIC(stdz.mod_thick_lai_seed_elev) - AIC(stdz.mod_thick_rand),
...
stdz.mod_thick_aic_arr
stdz.mod_thick_wi <-
  Weights(stdz.mod_thick_aic_arr$stdz.mod_thick_aic_val)
stdz.mod_thick_wi
stdz.mod_thick_aic_arr$wi <- stdz.mod_thick_wi
stdz.mod_thick_aic_arr <-
  stdz.mod_thick_aic_arr %>% dplyr::select(model_name_stdz.mod_thick_aic,
    stdz.mod_thick_aic_val,
    dAIC,
    wi)
...
stdz.mod_thick_aic_arr$R2c <-
  c(r1[2], r2[2], r3[2], r4[2], r5[2], r6[2], r7[2], r8[2], r9[2], r10[2], r11[2])
stdz.mod_thick_aic_arr$r2m <-
  c(r1[1], r2[1], r3[1], r4[1], r5[1], r6[1], r7[1], r8[1], r9[1], r10[1], r11[1])
...
stdz.mod_thick_fix <-
  data.frame(
    "factor" = stdz.mod_thick_fix_name,
    "slope" = stdz.mod_thick_fix_slope,
    "se" = stdz.mod_thick_fix_se,
    "trait" = stdz.mod_thick_fix_trait
  )
stdz.mod_thick_sort <-
  c("Elevation.code", "ISI", "LAI", "Comp.seed")
stdz.mod_thick_fix <-
  stdz.mod_thick_fix %>% slice(match(stdz.mod_thick_sort, stdz.mod_thick_fix_name))
stdz.mod_thick_fix$stdz.mod_thick_fix_name <-
  factor(stdz.mod_thick_fix$stdz.mod_thick_fix_name,
    levels = stdz.mod_thick_fix$stdz.mod_thick_fix_name)
stdz.mod_thick_fix
stdz.mod_thick_fix_plot1 <-
  ggplot() + geom_errorbar(
    data = stdz.mod_thick_fix,
    aes(
      x = stdz.mod_thick_fix_name,
      ymin = stdz.mod_thick_fix_slope - stdz.mod_thick_fix_se,
      ymax = stdz.mod_thick_fix_slope + stdz.mod_thick_fix_se,
      colour = stdz.mod_thick_fix_name
    )
  )
stdz.mod_thick_fix_plot2 <-
  stdz.mod_thick_fix_plot1 + geom_point(
    data = stdz.mod_thick_fix,
    aes(x = stdz.mod_thick_fix_name, y = stdz.mod_thick_fix_slope, colour = stdz.mod_thick_fix_name),
    size = 5
  )
stdz.mod_thick_fix_plot3 <-
  stdz.mod_thick_fix_plot2 + theme_bw() + theme(
    plot.background = element_blank(),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank()
  ) + theme(panel.border = element_blank()) + theme(
    axis.line.x = element_line(color = "black", size = 1),
    axis.line.y = element_line(color = "black", size = 1)
  ) + theme(legend.position = "right") + xlab("Fixed Effects") + ylab(expression(paste("Fixed Effect Slope")))
stdz.mod_thick_fix_plot4 <-
  stdz.mod_thick_fix_plot3 + theme(axis.text = element_text(size = 14),
    axis.title = element_text(size = 14)) + theme(legend.position = "none") + ggtitle("Leaf Thickness") + theme(axis.title.x = element_blank()) + geom_hline(yintercept = 0, linetype = 5) + annotate(
      "text",
      x = 4,
      y = 0,
      label = "NA",
      size = 10
    ) + ylim(-0.75, 0.75) +  scale_x_discrete(
      "stdz.mod_fvfm_fix_name",
      labels = c(
        "Elevation.code" = "Elevation.code",
        "ISI" = "ISI",
        "LAI" = "LAI",
        "Comp.seed" = "Herbaceous Plants"
      )
    )
stdz.mod_thick_fix_plot4
