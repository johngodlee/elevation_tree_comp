...
sed -i '10s/.*/{Site} \& {Mean elev.} \& {Precip. (mm y\\textsuperscript{-1})} \& { Mean temp. (\\textdegree{}C)} \& {Soil C (\\%)} \& {Soil N (\\%)} \& {Soil pH} \& {Trees ha\\textsuperscript{-1}} \\\\/' manuscript/include/site_char.tex
...
sed -i '10s/.*/{Response} \& {Fixed effects} \& {$\\Delta{}AIC_r$} \& {$W_i$} \& {$R^2_c$} \& {$R^2_m$} \\\\/' manuscript/include/best_mod_multi_output.tex
...
sed -i 's/leaf\\_chl/Chlorophyll-$\\alpha$/' manuscript/include/best_mod_multi_output.tex

sed -i 's/leaf\\_height\\_ratio/Leaf:height ratio/'  manuscript/include/best_mod_multi_output.tex

sed -i 's/leaf\\_area\\_cm2/Leaf area/'  manuscript/include/best_mod_multi_output.tex
