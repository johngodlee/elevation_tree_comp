...
sed -i '10s/.*/{Site} \& {Elev.} \& {Precip. (mm y\\textsuperscript{-1})} \& { Mean temp. (\\textdegree{}C)} \& {Soil C (\\%)} \& {Soil N (\\%)} \& {Soil pH} \& {Trees ha\\textsuperscript{-1}} \\\\/' manuscript/include/site_char.tex
